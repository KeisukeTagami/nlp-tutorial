...
auto main() -> int {
  ...
  auto train_dataset = torch::data::datasets::NLP()
  ...
  auto test_dataset = torch::data::datasets::NLP(torch::data::datasets::NLP::Mode::kTest)
    // .map(torch::data::transforms::Normalize<>(0.1307, 0.3081))
    .map(torch::data::transforms::Stack<>());
